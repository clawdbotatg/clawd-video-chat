...
| `com.clawd.stt-selftest` | The **hearing test** — every 20 min (passes outright if real room text reached the server in the last 90s — a busy call masks the marker) speaks a marker phrase into BlackHole 2ch and verifies it traverses the whole transcript pipeline (cable → SR → `/api/stt-log` → heartbeat file; `server.py` filters the marker so it never reaches the transcript or the brain). On failure: page self-reload → AppleScript tab reload → macOS notification. Silently skips when `:7900` is down (rig off) or the default input isn't 2ch. `stt-selftest.sh`. | — | `/tmp/stt-selftest.log` |
